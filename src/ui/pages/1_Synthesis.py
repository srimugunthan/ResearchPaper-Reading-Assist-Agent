"""Streamlit page: Multi-Paper Synthesis."""
import os
...
with st.sidebar:
    st.header("Settings")
    llm_provider = st.selectbox("LLM Provider", ["gemini", "ollama", "fake"], index=0, key="synth_llm")
    llm_model = st.text_input(
        "Model Name",
        value="gemini-2.0-flash" if llm_provider == "gemini" else "llama3",
        key="synth_model",
    )
    temperature = st.slider("Temperature", 0.0, 1.0, 0.1, step=0.05, key="synth_temp")
...
with col2:
    top_k = st.slider("Max chunks to retrieve", min_value=5, max_value=50, value=20, key="synth_topk")
